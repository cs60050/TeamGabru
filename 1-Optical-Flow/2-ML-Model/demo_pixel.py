...
basepath = "E:\\Acads\\._7th Sem\\Machine Learning\\Project\\Dataset\\UCSD_Anomaly_Dataset.v1p2\\UCSDped1\\Test"
...
def getFeaturesFromVideo(imagelist, taglist, out, clf, mag_threshold=1e-3, atom_shape=[10,10,5]):
	...
	prevgray = cv2.imread(imagelist[k], cv2.IMREAD_GRAYSCALE)
	# taglist.next()
	k+=1
	...
	while k<length:
		#Read next frame
		try:
			img = cv2.imread(imagelist[k], cv2.IMREAD_GRAYSCALE)
			k+=1
		except:
			print("done")
			break
		...
		# Read Tagged image
		# tag_img_ = cv2.imread(next(taglist), cv2.IMREAD_GRAYSCALE)
...
for testf in testfolders:
	...
	cnt = len(images)
	filename = "E:\\Acads\\._7th Sem\\Machine Learning\\Project\\TeamGabru\\ML-Model\\TrainedClassifiers\\50\\Ped1\\DecisionTree.pkl"
	...
	for i in range(2,cnt-1):
		...
		added = cv2.addWeighted( orig, 0.8, mask, 0.2, 0.0)
